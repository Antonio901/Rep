...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Разделы = МодульТекущиеДелаСервер.РазделыДляОбъекта("Обработка.НастройкаОтчетовОбАварийномЗавершенииРаботы");
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Форма          = "Обработка.НастройкаОтчетовОбАварийномЗавершенииРаботы.Форма.ЗапросНаСборИОтправкуОтчетовОбОшибках";
	КонецЦикла;
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Форма          = "Обработка.НастройкаОтчетовОбАварийномЗавершенииРаботы.Форма.ЗапросНаОтправкуОтчетовОбОшибках";
	КонецЦикла;
...
// Служебная процедура.
// Отправляет тестовый пакет.
// В тестовом пакете содержится идентификатор ИБ, Конфигурация и ее версия.
//
Процедура ОтправитьТестовыйПакет() Экспорт
	...
	Если НЕ ПараметрыЦентраМониторинга.ТестовыйПакетОтправлен И ПараметрыЦентраМониторинга.ЧислоПопытокОтправкиТестовогоПакета < 3
		И ЭтоГлавныйУзел() И ДатаЗапуска >= ПараметрыЦентраМониторинга.ОтправкаДанныхСледующееФормирование Тогда 
		...
		Попытка
			HTTPОтвет = ОтправитьДанныеМониторинга(Истина);
			Если HTTPОтвет.КодСостояния = 200 Тогда
