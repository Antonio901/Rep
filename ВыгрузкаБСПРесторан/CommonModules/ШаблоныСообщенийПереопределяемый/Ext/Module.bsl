...
Процедура ПриОпределенииНастроек(Настройки) Экспорт
	...
	Предмет = Настройки.ПредметыШаблонов.Найти(Метаданные.Документы.ЗаказПокупателя.ПолноеИмя(), "Имя");
	Предмет.ЗначенияПараметровСКД.Вставить("ОбращениеПолНеопределен", НСтр("ru='Уважаемый(ая)'"));
	...
	НовыйРеквизит.Тип = Тип("СправочникСсылка.Организации");
...
Процедура ПриПодготовкеШаблонаСообщения(Реквизиты, Вложения, НазначениеШаблона, ДополнительныеПараметры) Экспорт
	...
	Если НазначениеШаблона = "Документ.СчетНаОплатуПокупателю"
		ИЛИ НазначениеШаблона = "Документ.ЗаказПокупателя" Тогда
	...
	Если НазначениеШаблона = "ОповещениеКлиентаИзменениеЗаказа" Тогда
		ШаблоныСообщений.СформироватьСписокРеквизитовПоСКД(Реквизиты, Документы.ЗаказПокупателя.ПолучитьМакет("ДанныеШаблонаСообщений"));
	КонецЕсли;
...
Процедура ПриФормированииСообщения(Сообщение, НазначениеШаблона, ПредметСообщения, ПараметрыШаблона) Экспорт
	...
	Если ПараметрыШаблона.ТипШаблона = "Письмо"
		И (НазначениеШаблона = "Документ.СчетНаОплатуПокупателю"
		ИЛИ НазначениеШаблона = "Документ.ЗаказПокупателя") Тогда
