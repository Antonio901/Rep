...
Процедура ВыполнитьПроверку(Знач ПараметрыПроверки) Экспорт
	...
	Если Не ОбновлениеИнформационнойБазы.ОбъектОбработан(Проверка).Обработан Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьВедениеУчета(ИдентификаторРегламентногоЗадания = Неопределено) Экспорт
	...
	Иначе
		...
		Пока Результат.Следующий() Цикл
			...
			ПараметрыПроверки.Вставить("Наименование",                      Результат.Наименование);
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Проблемы = КонтрольВеденияУчета.СводнаяИнформацияПоВидамПроверок("СистемныеПроверки");
...
Функция НовыйВидПроверки(ПараметрыПроверки)
	...
	Если ЗначениеЗаполнено(ПараметрыПроверки.Наименование) Тогда
		НовыйВидПроверки.Наименование = ПараметрыПроверки.Наименование;
	Иначе
...
Процедура ДобавитьПроверки(Проверки)
	...
	Для Каждого Проверка Из Проверки Цикл
		...
		ПроверкаОбъект.Родитель      = КонтрольВеденияУчета.ПроверкаПоИдентификатору(Проверка.ИдентификаторГруппы);
...
Процедура ПодготовитьДанныеПроверки(Проверка, ПараметрыПроверки)
	...
	Параметры.Вставить("Наименование",           НСтр("ru='Системные проверки'"));
