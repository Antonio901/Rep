...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	Форма.Элементы.ГруппаОтправить.Подсказка = НСтр("ru = '<Тест>'");
	Если Форма.Параметры.КлючВарианта = "ПоВерсиям" Тогда
		...
		Если ОписаниеКоманды <> Неопределено И ОписаниеКоманды.Идентификатор = "ОтчетПоВерсиям" Тогда
			Форма.ФормаПараметры.Отбор.Вставить("Папка", ВладельцыФайлов(Форма.ФормаПараметры.Отбор.Ссылка));
		КонецЕсли;
	КонецЕсли;
	
	Команда = Форма.Команды.Добавить("Команда");
